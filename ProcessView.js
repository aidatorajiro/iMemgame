...
class ProcessView {
  constructor (pid) {
    ...
    Globals.lock()
    const regions = this.mem.getRegions()
  ...
  update () {
    ...
    const adj_rows = this.rows + 4
    const adj_cols = this.cols + 4

    for (let j = -adj_rows; j < adj_rows + 1; j++) {
      const data = this.getByteSync(charaX, charaY + j, adj_cols * 2 + 1)
      for (let i = -adj_cols; i < adj_cols + 1; i++) {
